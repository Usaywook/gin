...
import numpy as np
import carla
from carla2gym.carla.PythonAPI.agents.navigation.agent import Agent
from carla2gym.carla.PythonAPI.agents.navigation.local_planner_behavior import LocalPlanner, RoadOption
...
from carla2gym.carla.PythonAPI.agents.navigation.types_behavior import Cautious, Aggressive, Normal

from carla2gym.carla.PythonAPI.agents.tools.misc import get_speed, positive
import pdb
...
class BehaviorAgent(Agent):
    ...
    def update_information(self, world):
        ...
        if self.incoming_waypoint is None:
            if len(self._local_planner.waypoints_queue) == 0:
                print('no naviagated points!')
                self.incoming_waypoint = self._grp._dao.get_waypoint(self.vehicle.get_location()).next(self._sampling_resolution)[0]
            else:
                self.incoming_waypoint = self._local_planner.waypoints_queue[-1]
